...
export function ChatContainer({ id, savedMessages = [], query }: ChatProps) {
  const {
    messages,
    input,
    handleInputChange,
  ...
  const researchPath = API_PATHS.research.byChat(id)
  const { data: researchState, mutate: mutateResearch } = useSWR<ResearchState>(
    researchPath,
    async (url: string) => {
  ...
  const handleClearResearch = async (chatId: string, isCleared: boolean) => {
    try {
      await fetch(`/api/v1/chat/${chatId}/research`, {
        method: 'PUT',
  ...
  const handleDepthChange = async (chatId: string, currentDepth: number, maxDepth: number) => {
    try {
      await fetch(`/api/v1/chat/${chatId}/research/depth`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="flex min-h-screen">
      {messages.length > 0 && (
        <>
          {/* Research components will be moved to features/research */}
        </>
      )}
